...
class JSON {
	...
	private function _check_table_dir() {
		...
		if (!is_dir($this->get_db_dir()) && !mkdir($this->get_db_dir(), 0775, true)) {
			$cache = false;
			throw new Exception('Tidak dapat membuat direktori file ' . $this->get_db_dir());
		} elseif (!is_readable($this->get_db_dir()) || !is_writable($this->get_db_dir())) {
			if (!chmod($this->get_db_dir(), 0775)) {
				$cache = false;
				throw new Exception($this->get_db_dir() . ' harus dapat dibaca dan ditulis');
			}
		}
		$cache = true;
		return true;
